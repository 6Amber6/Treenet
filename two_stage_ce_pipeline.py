...
def train_classifier(model: nn.Module, train_loader: DataLoader, test_loader: DataLoader, epochs: int = 50, lr: float = 0.1) -> None:
...
def train_head_g(X_train: torch.Tensor, y_train: torch.Tensor, X_test: torch.Tensor, y_test: torch.Tensor,
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Two-stage CE optimization pipeline: train M1/M2, extract embeddings, train G")
