"""
Plain 10-class Adversarial Training baseline (no M1/M2 fusion)

Loss matches the fusion experiment (minus sub-head aux CE):
    Total = CE(clean 10-class head) + beta * TRADES(KL)

Training pipeline (mirrors your improved-fusion script):
  1) Build a LightResnet(10)
  2) CE warmup for N epochs (default 5)
  3) BatchNorm calibration on clean data
  4) Full adversarial training with TRADES (or MART)
  5) Clean & adversarial evaluation using the 10-class head

Why this file:
  - Gives an apples-to-apples baseline: what if we directly train a single
    10-class model with the SAME objective and attack settings as fusion.
"""
...
def build_lightresnet20_10() -> nn.Module:
    """Small ResNet (the same family used in the fusion code) for 10-way classification."""
...
def make_eval_attack(model: nn.Module, args):
    ...
    Uses the exact same attack hyper-parameters as training flags.
    """
...
def bn_calibration(model: nn.Module, loader: DataLoader, max_batches=200):
    ...
    Call after CE warmup and also optionally during adversarial training.
    """
    model.train()
...
# ------------------------------------------------
# CE warmup and TRADES loss (same structure as your fusion script, minus aux CE)
# ------------------------------------------------
def custom_trades_loss_plain(model: nn.Module,
                             x_natural, y, optimizer,
                             step_size=0.003, epsilon=0.031, perturb_steps=10,
                             beta=6.0):
    """
    TRADES on a plain 10-class model:
      total = CE(model(x), y) + beta * KL( model(x_adv) || model(x) )

    Notes:
      - We freeze BN running stats when crafting x_adv (model.eval())
      - We then switch to model.train() for the parameter update
    """
    criterion_kl = nn.KLDivLoss(reduction='sum')
    batch_size = x_natural.size(0)

    # ----- generate adversarial example -----
    model.eval()  # important: keep BN stats stable during inner PGD
    ...
    for _ in range(perturb_steps):
        ...
        # PGD step + L_inf projection + clip to image range
...
def train_plain10(model: nn.Module,
                  ...
                  warmup_epochs: int = 5):
    """
    Train a single 10-class model with:
      - CE warmup (warmup_epochs)
      - CosineAnnealingLR
      - TRADES (or MART) as requested by --trainer
    """
    weight_decay = getattr(args, 'weight_decay', 5e-4)

    # Simple single-group optimizer; you can mirror LR from fusion runs
    ...
    for ep in range(1, warmup_epochs + 1):
        ...
        if ep % 5 == 0 or ep == 1:
            ...
            adv_acc = eval_adv(model, test_loader, eval_atk)  # expect ~0 in warmup
    ...
    for ep in range(1, args.epochs_g + 1):
        ...
        for x, y in train_loader:
            ...
            if args.trainer == 'trades':
                loss_main = custom_trades_loss_plain(
                ...
                # MART expects model(x)->logits (this model already does)
                loss_val = mart_loss(
                    model, x, y, optimizer=opt,
                    beta=args.beta,
                    step_size=getattr(args, 'attack_step', 2/255),
...
# ------------------------------------------------
def main():
    # Reuse repo's parser_train (keeps your CLI consistent: --data/--attack-*)
    parse = parser_train()
