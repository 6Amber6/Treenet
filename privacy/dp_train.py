...
"""
DP-SGD training script using sampling rate only (no batch size argument).
Implements T1=T2, Total=T1+T3, and automatic noise multiplier from ε.
"""
...
def train_dp(model, loader, test_loader, steps, lr, sigma, C, device):
    model = model.to(device)
    optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9)
    for step in range(steps):
        ...
        if step % 10 == 0:
            ...
            print(f"Step {step}/{steps} | Acc={acc*100:.2f}%")
    return model
...
def main():
    ...
    parser.add_argument("--T3", type=int, required=True)
    ...
    print(f"Computed σ={sigma:.4f} for ε={args.epsilon}, δ={args.delta}, q={args.sampling_rate}")
    loaders = DataProcessor.create_data_loaders(args.data_dir, sampling_rate=args.sampling_rate)

    if args.train_all or args.train_4class:
        print("\n[Training 4-class (vehicles)]")
        model4 = DP4Classifier()
        train_dp(model4, loaders["vehicle_train"], loaders["vehicle_test"], args.T1, args.lr, sigma, args.max_grad_norm, device)

    if args.train_all or args.train_6class:
        print("\n[Training 6-class (animals)]")
        model6 = DP6Classifier()
        train_dp(model6, loaders["animal_train"], loaders["animal_test"], args.T1, args.lr, sigma, args.max_grad_norm, device)

    if args.train_all or args.train_10class:
        print("\n[Training 10-class (baseline)]")
        model10 = DP10Classifier()
        train_dp(model10, loaders["full_train"], loaders["full_test"], args.T3, args.lr, sigma, args.max_grad_norm, device)
