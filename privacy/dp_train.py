...
"""
Debugging version of DP-SGD training for hierarchical and baseline models.
Added diagnostics: label check, model output shape, gradient clipping stats.
"""
...
def train_dp(model, train_loader, test_loader, epochs, lr,
    ...
    print(f"[DEBUG] Unique labels: {yb.unique().tolist()}")
    xb = xb.to(device)
    with torch.no_grad():
        out = model(xb)
        if isinstance(out, tuple):
            out = out[0]
        print(f"[DEBUG] Output shape: {tuple(out.shape)}")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            preclip_norm, clip_ratio, grad_mean = dp_step_images(
                model, optimizer, x, y, noise_multiplier, max_grad_norm,
                return_debug=True
            )
            ...
            if batch_idx % 50 == 0:
                loss_val = F.cross_entropy(model(x)[0], y).item()
                print(f"Epoch {epoch}, Batch {batch_idx}, Loss {loss_val:.4f}, "
                      f"PreClip||g|| {preclip_norm:.2f}, ClipRatio {clip_ratio*100:.1f}%, "
                      f"GradMean {grad_mean:.4e}, ε={eps:.3f}, δ={delta:.1e}")

        train_acc = compute_accuracy(model, train_loader, device)
        test_acc = compute_accuracy(model, test_loader, device)
        print(f"Epoch {epoch}: Train {train_acc:.4f}, Test {test_acc:.4f}, ε={eps:.3f}")
...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str, default="./data")
    parser.add_argument("--output_dir", type=str, default="./results_debug")
    parser.add_argument("--batch_size", type=int, default=128)
    parser.add_argument("--lr", type=float, default=0.05)
    parser.add_argument("--epochs_4class", type=int, default=10)
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("============================================================")
    print("PER-SAMPLE DP-SGD TRAINING (Debug Mode)")
    print("============================================================")
    print(f"Device: {device}")
    print(f"sigma={args.noise_multiplier}, C={args.max_grad_norm}, delta={args.delta}")
    print(f"LR={args.lr}, batch={args.batch_size}")

    loaders = DataProcessor.create_data_loaders(args.data_dir, args.batch_size)
    m4_train, m4_test = loaders["vehicle_train"], loaders["vehicle_test"]

    print("\nTraining 4-class model (Debug)...")
    model4 = DP4Classifier()
    train_dp(model4, m4_train, m4_test, args.epochs_4class, args.lr,
             args.noise_multiplier, args.max_grad_norm, args.delta, device)
