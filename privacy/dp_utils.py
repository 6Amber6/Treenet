...
from privacy.dp_datasets import get_toy_dataset
...
class DataProcessor:
    @staticmethod
    def create_data_loaders(data_dir, batch_size=128):
        x_train, y_train, x_test, y_test = get_toy_dataset(4)  # 4-class toy set
        train_ds = TensorDataset(x_train, y_train)
        test_ds = TensorDataset(x_test, y_test)
        loaders = {
            "vehicle_train": DataLoader(train_ds, batch_size=batch_size, shuffle=True),
            "vehicle_test": DataLoader(test_ds, batch_size=batch_size),
            "animal_train": DataLoader(train_ds, batch_size=batch_size),
            "animal_test": DataLoader(test_ds, batch_size=batch_size),
            "full_train": DataLoader(train_ds, batch_size=batch_size),
            "full_test": DataLoader(test_ds, batch_size=batch_size),
        }
        return loaders
...
# DP-SGD core
# ------------------------
def dp_step_images(model, optimizer, x, y, noise_multiplier, max_grad_norm, return_debug=False):
    # forward + loss
    logits, _ = model(x)
    ...
    # compute preclip norm
    total_norm = torch.norm(
        torch.stack([p.grad.data.norm(2) for p in model.parameters() if p.grad is not None])
    )
    ...
    clip_ratio = float(clip_coef < 1.0)
    grad_mean = torch.cat([p.grad.flatten() for p in model.parameters() if p.grad is not None]).abs().mean().item()
    if return_debug:
        return preclip_norm, clip_ratio, grad_mean
    return preclip_norm
...
def compute_epsilon_opacus(noise_multiplier, sample_rate, steps, delta):
    if noise_multiplier == 0:
        return float("inf")
    return (sample_rate * steps) / (noise_multiplier + 1e-8)
