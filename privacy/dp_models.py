...
class DP4Classifier(SimpleCNN):
    def __init__(self):
        super().__init__(4)


class DP6Classifier(SimpleCNN):
    def __init__(self):
        super().__init__(6)


class DP10Classifier(SimpleCNN):
    def __init__(self):
        super().__init__(10)


class DPFusionModel(nn.Module):
    def __init__(self, emb_dim=32 * 7 * 7, num_classes=10):
        super().__init__()
        self.fc1 = nn.Linear(emb_dim * 2, 256)
        self.fc2 = nn.Linear(256, num_classes)

    def forward(self, emb4, emb6):
        x = torch.cat((emb4, emb6), dim=1)
        x = F.relu(self.fc1(x))
        logits = self.fc2(x)
        return logits
