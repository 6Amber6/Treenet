...
def trades_fusion_step(model: FusionWRN, x_natural, y, optimizer,
    ...
    # Craft with eval() so BN/dropout frozen
    logits_model.eval()
    with torch.no_grad():
        p_nat = F.softmax(logits_model(x_natural), dim=1)

    # PGD in normalized space; eps/step are in pixel scale -> we assume create_attack/train use same scale
    ...
    for _ in range(perturb_steps):
        ...
        # No extra clamp needed; inputs are normalized already
...
def train_fusion(model: FusionWRN, train_loader, test_loader, args, logger):
    ...
    # Disable head dropout for TRADES, freeze BN stats, train with TRADES
    ...
    for ep in range(warmup_epochs + 1, args.epochs_g + 1):
        ...
        # Unfreeze BN after 40 TRADES epochs to let stats adapt
