...
def train_fusion(model, train_loader, test_loader, args, logger):
    ...
    for ep in range(1, warmup_epochs + 1):
        ...
        ema.restore(model)
        logger.log(
            f"[Fusion-CE] Epoch {ep:03d} | Train Loss {total_loss/max(n,1):.4f} | Clean {clean:.4f}"
        )
    set_head_dropout_prob(model, 0.0)
    freeze_backbone_bn(model)
    atk_eval = make_eval_attack(model, args)
    ...
    for ep in range(warmup_epochs + 1, args.epochs_g + 1):
        ...
        ema.restore(model)
        logger.log(
            f"[Fusion-TRADES] Epoch {ep:03d} | Train Loss {total_loss/max(n,1):.4f} | Clean {clean:.4f} | Adv {adv:.4f}"
        )
